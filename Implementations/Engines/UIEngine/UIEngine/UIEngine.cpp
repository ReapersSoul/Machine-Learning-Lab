...
class UIEngine : public UIEngineInterface
{
	...
	void HandleGraphInputEvents() {
		try {
			...
			if (ImGui::BeginPopup("Background Context Menu")) {
				...
				if (SearchText.empty()) {
					for (auto node : NS_Node::Registrar::GetRegistrar()->GetTypeIDs()) {
						//if node is clicked
						if (ImGui::MenuItem(node.first.c_str())) {
							//add node to graph
							GraphEngine->GetGraphs()["main"]->GetMutex().lock();
							int uuid = GraphEngine->GetGraphs()["main"]->AddNode(NS_Node::Registrar::GetRegistrar()->Construct(node.second));
	...
	void DrawSettingsWindow() {
		if (ImGui::Begin("Settings")) {
			...
			for (auto& activation : NS_Activation::Registrar::GetRegistrar()->GetActivations())
			{
				ImGui::Text(activation.first.c_str());
			}
			ImGui::Separator();
			ImGui::Text("Available Losses:");
			for (auto& loss : NS_Loss::Registrar::GetRegistrar()->GetLosses())
			{
