...
ImRect GetContainingRectForCubicBezier(const CubicBezier& cb)
{
...
float EvalImplicitLineEq(const ImVec2& p1, const ImVec2& p2, const ImVec2& p)
{
    return (p2.y - p1.y) * p.x + (p1.x - p2.x) * p.y + (p2.x * p1.y - p1.x * p2.y);
}

int Sign(float val) { return int(val > 0.0f) - int(val < 0.0f); }

bool RectangleOverlapsLineSegment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2)
{
...
bool RectangleOverlapsBezier(const ImRect& rectangle, const CubicBezier& cubic_bezier)
{
    ImVec2 current =
    ...
    const ImOptionalIndex pin_idx = ImOptionalIndex())
{
    ...
    else
    {
        if (pin_idx.HasValue())
        {
            const int hovered_pin_flags = editor.Pins.Pool[pin_idx.Value()].TypeFlags;
...
void ClickInteractionUpdate(ImNodesEditorContext& editor)
{
    ...
    case ImNodesClickInteractionType_LinkCreation:
    {
        ...
        const bool link_creation_on_snap =
            GImNodes->HoveredPinIdx.HasValue() &&
            (editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()].TypeFlags &
...
void DrawNode(ImNodesEditorContext& editor, const int node_idx)
{
...

    {
        ...
        if ((GImNodes->Style.TypeFlags & ImNodesStyleFlags_NodeOutline) != 0)
        {
#if IMGUI_VERSION_NUM < 18200
    ...
    const int                  node_idx)
{
    ...
    pin.Type = type;
    pin.Shape = shape;
    pin.TypeFlags = GImNodes->CurrentAttributeFlags;
...
static void MiniMapUpdate()
{
    ...
    int catagory_flags = ImGuiWindowFlags_NoBackground;
    ImGui::SetCursorScreenPos(editor.MiniMapRectScreenSpace.Min);
    ImGui::BeginChild("minimap", editor.MiniMapRectScreenSpace.GetSize(), false, catagory_flags);
...
template<typename V>
void SelectObject(const ImObjectPool<V>& objects, ImVector<int>& selected_indices, const int ID)
{
...
template<typename V>
bool IsObjectSelected(const ImObjectPool<V>& objects, ImVector<int>& selected_indices, const int ID)
{
    const int idx = ObjectPoolFind(objects, ID);
...
ImNodesStyle::ImNodesStyle()
    : GridSpacing(32.f), NodeCornerRounding(4.f), NodePadding(8.f, 8.f), NodeBorderThickness(1.f),
      ...
      MiniMapOffset(4.0f, 4.0f), TypeFlags(ImNodesStyleFlags_NodeOutline | ImNodesStyleFlags_GridLines),
      Colors()
{
}
...
void BeginNodeEditor()
{
    ...
    ImGui::BeginGroup();
    {
...

        {
            ...
            if (GImNodes->Style.TypeFlags & ImNodesStyleFlags_GridLines)
            {
...
void Link(const int ID, const int start_attr_id, const int end_attr_id)
{
    ...
    if ((editor.ClickInteraction.Type == ImNodesClickInteractionType_LinkCreation &&
         editor.Pins.Pool[link.EndPinIdx].TypeFlags & ImNodesAttributeFlags_EnableLinkCreationOnSnap &&
