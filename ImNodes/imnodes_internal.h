...
struct ImOptionalIndex
{
    ...
    bool HasValue() const { return _Index != INVALID_INDEX; }
    ...
    ImOptionalIndex& operator=(const int value)
    {
        _Index = value;
        return *this;
    }
    ...
    bool operator==(const ImOptionalIndex& rhs) const { return _Index == rhs._Index; }

    bool operator==(const int rhs) const { return _Index == rhs; }
...
struct ImPinData
{
    ...
    ImPinData(const int pin_id)
        : Id(pin_id), ParentNodeIdx(), AttributeRect(), Type(ImNodesAttributeType_None),
          Shape(ImNodesPinShape_CircleFilled), Pos(), TypeFlags(ImNodesAttributeFlags_None),
          ColorStyle()
    {
    }
};
...
{
static ImNodesEditorContext& EditorContextGet()
{
...
template<typename V>
static int ObjectPoolFind(const ImObjectPool<V>& objects, const int ID)
{
...
template<typename V>
static void ObjectPoolUpdate(ImObjectPool<V>& objects)
{
...
template<typename V>
static void ObjectPoolReset(ImObjectPool<V>& objects)
{
...
template<typename V>
static int ObjectPoolFindOrCreateIndex(ImObjectPool<V>& objects, const int ID)
{
    ...
    if (index == -1)
    {
        ...
        IM_PLACEMENT_NEW(objects.Pool.Data + index) V(ID);
...
template<typename V>
static V& ObjectPoolFindOrCreateObject(ImObjectPool<V>& objects, const int ID)
{
